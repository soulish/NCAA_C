...
int main(int argc,char *argv[]){
    ...
    if (outFileName != "") {
        ...
        std::unordered_map<std::string, TH1F *> predicted_wavgsA;
        std::unordered_map<std::string, TH1F *> predicted_wavgsB;
        ...
        for (std::string &s : stats) {
            ...
            sprintf(histName, "predicted_wavgA_%s", (boost::replace_all_copy(s, ".", "").c_str()));
            predicted_wavgsA.emplace(s, new TH1F(histName, s.c_str(), 150, 0, 150));
            ...
            sprintf(histName, "predicted_wavgB_%s", (boost::replace_all_copy(s, ".", "").c_str()));
            predicted_wavgsB.emplace(s, new TH1F(histName, s.c_str(), 150, 0, 150));
        }
        if (useHistogramsFile) {
            ...
            predicted_wavgsA.emplace("gameScore", new TH1F("predicted_wavgA_gameScore", "gameScore", 150, 0, 150));
            ...
            predicted_wavgsB.emplace("gameScore", new TH1F("predicted_wavgB_gameScore", "gameScore", 150, 0, 150));
        }
        ...
        boost::gregorian::date evaluationDateA = seasonInfo->get(teamA->getYear(), evaluationDay);
        boost::gregorian::date evaluationDateB = seasonInfo->get(teamB->getYear(), evaluationDay);

        //cycle through the averages hash for teamA
        for (auto &average : averageHashA) {
            if (average.second->getDate() > evaluationDateA) continue;
            ...
            for (std::string &s : stats) {
                if (boost::contains(s, ".p") == 1) {
                    ...
                    avgsA_per_time[s]->SetBinContent(ind, average.second->getValue(s) /
                                                          (double) average.second->getNum_games());
        ...
        for (auto &average : averageHashB) {
            if (average.second->getDate() > evaluationDateB) continue;
        ...
        predicted_wavgsA["oor.p"]->SetBinContent(140, predictionsA["oor.p"] * waA->getValue("oor.p"));
        predicted_wavgsB["oor.p"]->SetBinContent(140, predictionsB["oor.p"] * waB->getValue("oor.p"));
        predicted_wavgsA["oefg.p"]->SetBinContent(140, predictionsA["oefg.p"] * waA->getValue("oefg.p"));
        predicted_wavgsB["oefg.p"]->SetBinContent(140, predictionsB["oefg.p"] * waB->getValue("oefg.p"));
        predicted_wavgsA["oftmr.p"]->SetBinContent(140, predictionsA["oftmr.p"] * waA->getValue("oftmr.p"));
        predicted_wavgsB["oftmr.p"]->SetBinContent(140, predictionsB["oftmr.p"] * waB->getValue("oftmr.p"));
        ...
        if (useHistogramsFile) {
            ...
            predicted_wavgsA["gameScore"]->SetBinContent(140, gamePctA);
            predicted_wavgsB["gameScore"]->SetBinContent(140, gamePctB);
            predicted_wavgsA["gameScore"]->SetBinError(140, gamePctA_err);
            predicted_wavgsB["gameScore"]->SetBinError(140, gamePctB_err);

            if (gameScoreA >= 0)
                std::cout << teamNameA << " is " << doubleFormatter(gamePctA * 100, 1) << "% likely to win the game" <<
                std::endl;
            else
                std::cout << teamNameB << " is " << doubleFormatter(gamePctB * 100, 1) << "% likely to win the game" <<
                std::endl;
        }
        else {
            if (gameScoreA >= 0)
                std::cout << teamNameA << " is favored to win the game with a gameScore of " <<
                doubleFormatter(gameScoreA, 3) << std::endl;
            else
                std::cout << teamNameB << " is favored to win the game with a gameScore of " <<
                doubleFormatter(gameScoreB, 3) << std::endl;
        }
